"""FGVC-Aircraft under the free-grain setting (Aircraft-F), generating superpixels.

See `aircraft_partial.py` for the encoding of the given labels.
"""
...
class FGVCAircraft_Hier(VisionDataset):
    def __init__(self,
        ...
        if is_train:
            if texts:
                ...
                # HSG：聚合家族级(70)/制造商级(30)文本语义（subord_labels 带 +29 偏移，减 30 得 0-based 家族）
                cap_t = torch.from_numpy(self.cap_embs).float()
                fam_t = torch.tensor(self.subord_labels, dtype=torch.long) - 30
                ord_t = torch.tensor(self.basic_labels, dtype=torch.long)
                z_family = torch.zeros(70, cap_t.size(1)); cf = torch.zeros(70)
                z_family.index_add_(0, fam_t, cap_t); cf.index_add_(0, fam_t, torch.ones(cap_t.size(0)))
                z_family = z_family / cf.clamp_min(1).unsqueeze(1)
                z_order = torch.zeros(30, cap_t.size(1)); co = torch.zeros(30)
                z_order.index_add_(0, ord_t, cap_t); co.index_add_(0, ord_t, torch.ones(cap_t.size(0)))
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any, Any]:
        ...
        if self.texts:
            return sample, segments, target, self.fine_labels[index], self.subord_labels[index] - 30, self.basic_labels[index], self.cap_embs[index]
        else:
            return sample, segments, target, self.fine_labels[index], self.subord_labels[index] - 30, self.basic_labels[index]
