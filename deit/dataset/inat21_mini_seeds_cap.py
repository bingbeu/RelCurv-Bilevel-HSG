"""iNat21-Mini under the free-grain setting (iNat21-F), generating superpixels.

See `inat21_mini_cap.py` for the encoding of the given labels.
"""
    ...
    def __init__(self,
        ...
        if is_train:
            if texts:
                ...
                # HSG：聚合科级(1103)/目级(273)文本语义（标注已给 0-based 标签，冻结）
                cap_t = torch.from_numpy(self.cap_embs).float()
                fam_t = torch.tensor(self.subord_label_list, dtype=torch.long)
                ord_t = torch.tensor(self.basic_label_list, dtype=torch.long)
                z_family = torch.zeros(1103, cap_t.size(1)); cf = torch.zeros(1103)
                ...
                z_order = torch.zeros(273, cap_t.size(1)); co = torch.zeros(273)
