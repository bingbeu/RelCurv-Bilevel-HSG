    ...
    def __init__(self,
        ...
        if is_train:
            if texts:
                ...
                # HSG：沿分类树聚合出家族级(70)/制造商级(30)文本语义（冻结，训练期层级接地用）
                #   变体 caption -> 家族语义 -> 制造商语义，每个层级拿到自己粒度的文本监督
                cap_t = torch.from_numpy(self.cap_embs).float()                 # (N,512)
                fine_t = torch.tensor(self.fine_labels, dtype=torch.long)        # (N,) 变体 0..99
                z_species = torch.zeros(100, cap_t.size(1))
                cnt = torch.zeros(100)
                ...
                fam_of = torch.tensor([trees[s][1] - 1 for s in range(100)], dtype=torch.long)
                z_family = torch.zeros(70, cap_t.size(1))
                cf = torch.zeros(70)
                z_family.index_add_(0, fam_of, z_species)
                cf.index_add_(0, fam_of, torch.ones(100))
                z_family = z_family / cf.clamp_min(1).unsqueeze(1)

                ord_of_fam = {}
                for s in range(100):
                    ord_of_fam.setdefault(trees[s][1] - 1, trees[s][2] - 1)
                ord_of = torch.tensor([ord_of_fam[f] for f in range(70)], dtype=torch.long)
                z_order = torch.zeros(30, cap_t.size(1))
                co = torch.zeros(30)
                z_order.index_add_(0, ord_of, z_family)
                co.index_add_(0, ord_of, torch.ones(70))
